...
class OrderControllerTest {
    ...
    @Test
    fun `getAllOrders should propagate exception when service fails`() = runTest {
        // Given
        val errorMessage = "Error al recuperar órdenes del usuario"
        coEvery { orderService.findAllOrdersForCurrentUser() } returns flow {
            throw RuntimeException(errorMessage)
        }

        // When & Then
        val exception = assertThrows<RuntimeException> {
            orderController.getAllOrders().toList()
        }
    ...
    @Test
    fun `getContainersByOrderId should wrap exception with descriptive message when service fails`() = runTest {
        // Given
        val purchaseNumber = "123L"

        coEvery { orderService.findContainersByOrderId(purchaseNumber) } returns flow {
            throw RuntimeException("Error de base de datos")
        }

        // When & Then
        assertThrows<RuntimeException> {
            orderController.getContainersByOrderId(purchaseNumber).toList()
        }
    }
}
