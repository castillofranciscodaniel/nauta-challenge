...
class OrderContainerRepositoryImplTest {
    ...
    @Test
    fun throwsRepositoryExceptionWhenExistsByOrderIdAndContainerIdFails() = runTest {
        val orderId = 1L
        val containerId = 2L

        every { orderContainerDao.existsByOrderIdAndContainerId(orderId, containerId) }.throws(RuntimeException("Database connection error"))

        val exception = assertFailsWith<RepositoryException> {
            orderContainerRepository.existsByOrderIdAndContainerId(orderId, containerId)
        }

        assertTrue(exception.message!!.contains("Error al verificar relación entre orden y contenedor"))
    }
}