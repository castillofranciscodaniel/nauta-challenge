...
class ContainerServiceTest {
    ...
    @Test
    fun devuelveContainerExistenteSiYaEstáAlmacenado(): Unit = runTest {
        // Arrange
        val bookingId = 1L
    ...
    @Test
    fun guardaContainerCuandoNoExiste(): Unit = runTest {
        // Arrange
        val bookingId = 1L
        val containerNumber = "CONT12345"
    ...
    @Test
    fun procesaMultiplesContainersCorrectamente(): Unit = runTest {
        // Arrange
        val bookingId = 1L
    ...
    @Test
    fun `findAllContainersForCurrentUser returns containers for user's bookings`() = runTest {
        // Arrange
        val userId = 100L
        ...
        coEvery { userLoggedService.getCurrentUser() } returns currentUser
    ...
    @Test
    fun `findAllContainersForCurrentUser returns empty flow when user has no containers`() = runTest {
        // Arrange
        val userId = 100L
        val currentUser = User(id = userId, email = "user@example.com", password = "password")
    ...
    @Test
    fun `findOrdersByContainerId returns orders for a specific container`() = runTest {
        // Arrange
        val userId = 100L
    ...
    @Test
    fun `findOrdersByContainerId returns empty flow when no orders found`() = runTest {
        // Arrange
        val userId = 100L
        val containerId = "CONT-001"
        val currentUser = User(id = userId, email = "user@example.com", password = "password")
    ...
    @Test
    fun manejaExcepcionAlGuardarContenedor(): Unit = runTest {
        // Arrange
        val bookingId = 1L
        val containerNumber = "CONT12345"
        val container = Container(id = null, containerNumber = containerNumber, bookingId = null)
        val containers = listOf(container)

        coEvery { containerRepository.findByContainerNumberAndBookingId(containerNumber, bookingId) } returns null
        coEvery { containerRepository.save(container.copy(bookingId = bookingId)) } throws RuntimeException("Error de base de datos")

        // Act & Assert
        assertFailsWith<RuntimeException>("Error de base de datos") {
            containerService.saveContainersForBooking(containers, bookingId)
        }
    }

    @Test
    fun manejaExcepcionAlVerificarContenedorExistente(): Unit = runTest {
        // Arrange
        val bookingId = 1L
        val containerNumber = "CONT12345"
        val container = Container(id = null, containerNumber = containerNumber, bookingId = null)
        ...
        } throws RuntimeException("Error al consultar base de datos")

        // Act & Assert
        assertFailsWith<RuntimeException>("Error al consultar base de datos") {
            containerService.saveContainersForBooking(containers, bookingId)
        }
    }

    @Test
    fun manejaListaVaciaDeContenedores(): Unit = runTest {
        // Arrange
        val bookingId = 1L
    ...
    @Test
    fun manejaExcepcionAlObtenerContenedoresDelUsuario(): Unit = runTest {
        // Arrange
        val userId = 100L
        val currentUser = User(id = userId, email = "user@example.com", password = "password")

        coEvery { userLoggedService.getCurrentUser() } returns currentUser
        every { containerRepository.findAllByUserId(userId) } throws RuntimeException("Error al obtener contenedores")

        // Act & Assert
        assertFailsWith<RuntimeException>("Error al obtener contenedores") {
            containerService.findAllContainersForCurrentUser().toList()
        }
    }

    @Test
    fun manejaExcepcionAlObtenerOrdenes(): Unit = runTest {
        // Arrange
        val userId = 100L
        val containerId = "CONT-001"
        val currentUser = User(id = userId, email = "user@example.com", password = "password")
        ...
        coEvery { userLoggedService.getCurrentUser() } returns currentUser
        every {
            orderRepository.findOrdersByContainerIdAndUserId(containerId, userId)
        } throws RuntimeException("Error al obtener órdenes")

        // Act & Assert
        assertFailsWith<RuntimeException>("Error al obtener órdenes") {
            containerService.findOrdersByContainerId(containerId).toList()
        }
    }

    @Test
    fun manejaExcepcionAlObtenerFacturas(): Unit = runTest {
        // Arrange
        val userId = 100L
        val containerId = "CONT-001"
        val currentUser = User(id = userId, email = "user@example.com", password = "password")
        val order = Order(id = 1L, purchaseNumber = "PO-001", bookingId = 1L)

        coEvery { userLoggedService.getCurrentUser() } returns currentUser
        every { orderRepository.findOrdersByContainerIdAndUserId(containerId, userId) } returns flowOf(order)
        every { invoiceService.findAllByOrderId(order.id!!) } throws RuntimeException("Error al obtener facturas")

        // Act & Assert
        assertFailsWith<RuntimeException>("Error al obtener facturas") {
            containerService.findOrdersByContainerId(containerId).toList()
        }
    }

    @Test
    fun ordenesSinFacturasSonDevueltasCorrectamente(): Unit = runTest {
        // Arrange
        val userId = 100L
        val containerId = "CONT-001"
        val currentUser = User(id = userId, email = "user@example.com", password = "password")
        ...
        coEvery { userLoggedService.getCurrentUser() } returns currentUser
        every { orderRepository.findOrdersByContainerIdAndUserId(containerId, userId) } returns flowOf(order)
        every { invoiceService.findAllByOrderId(order.id!!) } returns flowOf()
        ...
        assertEquals(0, result[0].invoices.size)
    }
}