...
class OrderServiceTest {
    ...
    fun `findAllOrdersForCurrentUser should return orders with invoices`(): Unit = runTest {
        ...
        coEvery { userLoggedService.getCurrentUser() } returns user
        ...
        // Verify
        coVerify { userLoggedService.getCurrentUser() }
    ...
    fun `findContainersByOrderId should return containers for authorized order`() = runTest {
        ...
        // Verify
        coVerify(exactly = 1) { userLoggedService.getCurrentUser() }
    ...
    @Test
    fun `findContainersByOrderId should return empty flow when throw an exception`() = runTest {
        // Arrange
        val userId = 1L
        val purchaseNumber = "PO-123"
        ...
        coEvery { containerRepository.findContainersByPurchaseNumberAndUserId(purchaseNumber, userId) }.throws(
            RepositoryException("Error retrieving containers", Exception("Database error"))
        )

        // Assert
        assertFailsWith <RepositoryException>("Error retrieving containers") {
            orderService.findContainersByOrderId(purchaseNumber).toList()
        }
    ...
    @Test
    fun `throws exception when orderRepository findByPurchaseNumberAndBookingId fails`() = runTest {
        // Arrange
        val bookingId = 1L
        val purchaseNumber = "PO-123"
        val order = Order(
        ...
        } throws RepositoryException("Error finding order")

        // Act & Assert
        val exception = assertFailsWith<Exception> {
            orderService.saveOrdersForBooking(listOf(order), bookingId)
        }

        assertEquals("Error finding order", exception.message)
    }

    @Test
    fun `throws exception when orderRepository save fails`() = runTest {
        // Arrange
        val bookingId = 1L
        val purchaseNumber = "PO-123"
        val order = Order(
        ...
        } throws RepositoryException("Error saving order")
        ...
        assertEquals("Error saving order", exception.message)
    }

    @Test
    fun `throws exception when invoiceService saveInvoicesForOrder fails`() = runTest {
        // Arrange
        val bookingId = 1L
        val purchaseNumber = "PO-123"
        val invoice = Invoice(id = null, invoiceNumber = "INV-001", orderId = null)
        ...
        coEvery {
            invoiceService.saveInvoicesForOrder(any(), any())
        } throws Exception("Error saving invoices")
        ...
        assertEquals("Error saving invoices", exception.message)
    }

    @Test
    fun `throws exception when findAllOrdersForCurrentUser fails to get current user`() = runTest {
        // Arrange
        coEvery {
            userLoggedService.getCurrentUser()
        } throws Exception("User not authenticated")

        // Act & Assert
        val exception = assertFailsWith<Exception> {
            orderService.findAllOrdersForCurrentUser().toList()
        }
    ...
    @Test
    fun `throws exception when findAllOrdersForCurrentUser fails to retrieve orders`() = runTest {
        // Arrange
        val currentUser = User(id = 1L, email = "test@example.com", password = "password")
        ...
        coEvery {
            orderRepository.findAllByUserId(currentUser.id!!)
        } returns flow { throw RepositoryException("Error retrieving orders") }
        ...
        assertEquals("Error retrieving orders", exception.message)
    }

    @Test
    fun `throws exception when findContainersByOrderId fails`() = runTest {
        // Arrange
        val purchaseNumber = "PO-123"
        val currentUser = User(id = 1L, email = "test@example.com", password = "password")
        ...
        coEvery {
            containerRepository.findContainersByPurchaseNumberAndUserId(purchaseNumber, currentUser.id!!)
        } returns flow { throw RepositoryException("Error retrieving containers") }
        ...
        assertEquals("Error retrieving containers", exception.message)
    }
}