...
class AuthServiceTest {
    ...
    @Test
    fun `propaga excepción cuando userRepository save falla durante el registro`() = runTest {
        // Arrange
        val request = RegisterRequestDto(email = "test@example.com", password = "password")
        val encodedPassword = "encodedPassword"
        ...
        coEvery { userRepository.existsByEmail("test@example.com") } returns false
        every { passwordEncoder.encode("password") } returns encodedPassword
        coEvery { userRepository.save(any()) } throws RuntimeException("Error de base de datos")
        ...
        assertEquals("Error de base de datos", exception.message)
    }

    @Test
    fun `propaga excepción cuando jwtTokenProvider generateToken falla durante el registro`() = runTest {
        // Arrange
        val request = RegisterRequestDto(email = "test@example.com", password = "password")
        val encodedPassword = "encodedPassword"
        val userWithId = User(id = 1, email = "test@example.com", password = encodedPassword)
        ...
        every { passwordEncoder.encode("password") } returns encodedPassword
        coEvery { userRepository.save(any()) } returns userWithId
        every { jwtTokenProvider.generateToken("test@example.com", 1) } throws RuntimeException("Error al generar token")
        ...
        assertEquals("Error al generar token", exception.message)
    }

    @Test
    fun `propaga excepción cuando jwtTokenProvider generateToken falla durante el login`() = runTest {
        // Arrange
        val request = LoginRequestDto(email = "test@example.com", password = "password")
        val user = User(id = 1, email = "test@example.com", password = "encodedPassword")
    ...
    @Test
    fun `propaga excepción cuando userRepository findByEmail falla durante el login`() = runTest {
        // Arrange
        val request = LoginRequestDto(email = "test@example.com", password = "password")

        coEvery { userRepository.findByEmail("test@example.com") } throws RuntimeException("Error de base de datos")
    ...
    @Test
    fun `propaga excepción cuando passwordEncoder matches falla durante el login`() = runTest {
        // Arrange
        val request = LoginRequestDto(email = "test@example.com", password = "password")
        val user = User(id = 1, email = "test@example.com", password = "encodedPassword")

        coEvery { userRepository.findByEmail("test@example.com") } returns user
        every { passwordEncoder.matches("password", "encodedPassword") } throws RuntimeException("Error al verificar contraseña")
        ...
        assertEquals("Error al verificar contraseña", exception.message)
    }
}